...
inline bool doesMac(uint8_t opcode, uint8_t mem){
	const uint8_t coef = getCoef(opcode);
	const uint8_t op = getOp(opcode);

	// if coef is 0 the operation is +=, the accumulator from the previous step is kept.
	// for op 0x30, coef is used for selecting inputs to the MAC, so it's kept out of this.
	if (op != 0x30 && coef == 0 && !getClr(op, mem, coef)) {
		return false;
	}
	// If it does clear the acc, the previous result is changed
	// so even if the result of MAC doesn't make sense, it can be used later.	
	if((mem & 0x10)){
		return true;
	}
	switch (op) {
		case 0x34:
			// mem is used as an op code, so anything not setting mulInputA will be garbage.
			if (mem >= 0xa0 && mem < 0xb0) {
              return false;
            } else if (mem >= 0xc0) {
				switch (mem & 0xf) {
...
inline const char* getAddressComment(uint32_t address, const uint8_t* intmem, bool clr) {
	...
	switch (address) {
		...
		case 0x044a: return "\n# mix * 70 / 32\n";
		case 0x044c: return "\n# Stores detune to iram[0x13] and mulcoeff[0]\n";
...
inline uint16_t findPrevAccInstruction(uint16_t address, bool acc, const uint8_t* intmem) {
	...
	const uint8_t mem = getMem(opcode);
	if (doesMac(opcode, mem) && getAccumulator(getOp(opcode), mem, getCoef(opcode), getShiftbits(opcode)) == acc) {
		return address;
	}
...
inline const char* getOpcodeDesc(uint16_t address, bool lastWasOp30, const uint8_t* intmem) {
    ...
    const bool acc = getAccumulatorFromAddress(address, intmem);
    ...
    switch (opc) {
        case 0x00: snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(getMulInputAFromMem(mem), coeff, shift)); return buf;
        case 0x04: snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(getMulInputAFromMem(mem), coeff, shift)); return buf;
        case 0x08: snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(satAStr, coeff, shift)); return buf;
        case 0x0C: snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(satBStr, coeff, shift)); return buf;
        case 0x10: snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(getMulInputAFromMem(mem), coeff, shift)); return buf;
        case 0x14: snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(getMulInputAFromMem(mem), coeff, shift)); return buf;
        case 0x18: snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(satAStr, coeff, shift)); return buf;
        case 0x1C: snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(satBStr, coeff, shift)); return buf;
        //case 0x20: snprintf(buf, sizeof(buf), "%s += gram[0x%02x] * %d >> %d", acc, mem, coeff, gramShift); return buf;
        case 0x20: snprintf(buf, sizeof(buf), "%s %s= gram[0x%02x]", accChar, clrChar, mem); return buf;
        //case 0x24: snprintf(buf, sizeof(buf), "%s  = gram[0x%02x] * %d >> %d", acc, mem, coeff, gramShift); return buf;
        case 0x24: snprintf(buf, sizeof(buf), "%s %s= gram[0x%02x]", accChar, clrChar, mem); return buf;
        ...
        case 0x30: {
            ...
            if ((coeff & 8) && !weird) {
              pos += getPrefix(buf, pos);
              //pos += snprintf(buf + pos, sizeof(buf) - pos, "mulB *= -1 (invert)\n");
			  pos += snprintf(buf + pos, sizeof(buf) - pos, "Flip mulB around 0\n");
            }
            if ((coeff & 16) && !weird) {
              pos += getPrefix(buf, pos);
              pos += snprintf(buf + pos, sizeof(buf) - pos, "Flip mulB around midpoint between 0 and MAX (or -MAX and 0)\n");
              //pos += getPrefix(buf, pos);
              //pos += snprintf(buf + pos, sizeof(buf) - pos, "if(mulB < 0) mulB = ~(mulB & 0x7fffff) (flip around midpoint between -MAX and 0\n");
            }
        ...
        case 0x34: {
            if (mem >= 0xa0 && mem < 0xb0) {
              ...
              switch (mem & 0xf)
              {
                ...
                case 0xf: snprintf(buf, sizeof(buf), "iram[0x%02x] = eram.eramReadLatch (also sets mulInputA_24 but maybe not used?)", mem | 0xf0); return buf;
                default:
        ...
        case 0x38: snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(satAStr, coeff, shift)); return buf;
        //case 0x38: return "";
        case 0x3C: snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(satBStr, coeff, shift)); return buf;
        //case 0x3C: return "";
        case 0x40: snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(rawAStr, coeff, shift)); return buf;
        case 0x44: snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(rawAStr, coeff, shift)); return buf;
        case 0x48: { static char rectSatA[80]; snprintf(rectSatA, sizeof(rectSatA), "rect(%s)", satAStr); snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(rectSatA, coeff, shift)); return buf; }
        case 0x4C: { static char rectSatA[80]; snprintf(rectSatA, sizeof(rectSatA), "rect(%s)", satAStr); snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(rectSatA, coeff, shift)); return buf; }
        case 0x50: return "setcondition true, clear A. Some skipfield magic";
        case 0x54: return "N/A";
        case 0x58: snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(satAStr, coeff, shift)); return buf;
        case 0x5C: snprintf(buf, sizeof(buf), "%s %s= %s", accChar, clrChar, getMACString(satBStr, coeff, shift)); return buf;
        case 0x60:
        case 0x64:
        case 0x70:
        case 0x74: snprintf(buf, sizeof(buf), "%s %s= (Pos: 1 - mulA, Neg: |mulA|) * %d", accChar, clrChar, coeff); return buf;
        case 0x68: // TODO: get mulA
        case 0x6C: snprintf(buf, sizeof(buf), "%s %s= (1-|%s|) * %d", accChar, clrChar, getMulInputAFromMem(mem), coeff); return buf;
        case 0x78:
        case 0x7C: snprintf(buf, sizeof(buf), "%s %s= |%s| * %d >> %d", accChar, clrChar, satAStr, coeff, shift); return buf;
